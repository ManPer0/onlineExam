...
export default {
  ...
  methods: {
    //获取当前所有考试信息
    getExamInfo() {
      console.log("-----"+this.$cookies.get("token-202235720346"))
      this.$axios(`/api/paper/exams/${this.pagination.current}/${this.pagination.size}`).then(res => {
    ...
    //搜索试卷
    search() {
      this.$axios('/api/paper/exams').then(res => {
