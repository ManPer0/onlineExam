...
export default {
  ...
  methods: {
    getTeacherInfo() {
      //分页查询所有试卷信息
      this.$axios(`/api/role/teachers/${this.pagination.current}/${this.pagination.size}`).then(res => {
    ...
    checkGrade(teacherId) { //修改教师信息
      ...
      this.$axios(`/api/role/teacher/${teacherId}`).then(res => {
    ...
    deleteById(teacherId) { //删除当前学生
      this.$confirm("确定删除当前教师吗？删除后无法恢复","Warning",{
        ...
        this.$axios({
          url: `/api/role/teacher/${teacherId}`,
          method: 'delete',
        }).then(res => {
          this.getTeacherInfo()
        })
