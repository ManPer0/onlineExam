...
export default {
  ...
  methods: {
    getStudentInfo() {
      //分页查询所有试卷信息
      this.$axios(`/api/role/students/${this.pagination.current}/${this.pagination.size}`).then(res => {
    ...
    checkGrade(studentId) { //修改学生信息
      ...
      this.$axios(`/api/role/student/${studentId}`).then(res => {
    ...
    deleteById(studentId) { //删除当前学生
      this.$confirm("确定删除当前学生吗？删除后无法恢复","Warning",{
        ...
        this.$axios({
          url: `/api/role/student/${studentId}`,
          method: 'delete',
        }).then(res => {
          this.getStudentInfo()
        })
