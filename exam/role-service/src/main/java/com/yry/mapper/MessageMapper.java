...
public interface MessageMapper extends BaseMapper<Message> {
    ...
    @Results({
            @Result(property = "replays", column = "temp_id",many = @Many(select = "com.yry.mapper.ReplayMapper.findAllById"))
    })
    IPage<Message> findAll(Page page);
//
//    @Select("select id,title,content,time from message where id = #{id}")
//    @Results({
//            @Result(property = "replays", column = "id",many = @Many(select = "com.yry.mapper.ReplayMapper.findAllById"))
//    })
